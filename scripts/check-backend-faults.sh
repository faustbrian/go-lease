...
postgres_name=go-lease-postgres-faults
postgres_replica_name=go-lease-postgres-replica-faults
valkey_name=go-lease-valkey-faults
valkey_replica_name=go-lease-valkey-replica-faults
valkey_secure_name=go-lease-valkey-secure-faults
network_name=go-lease-backend-faults
postgres_replica_volume=go-lease-postgres-replica-data
valkey_secure_volume=go-lease-valkey-secure-data
postgres_port=55432
...
for _ in {1..100}; do
  replica_counter="$({
    docker exec "$valkey_replica_name" valkey-cli --scan \
      --pattern 'go-lease-continuity:*:counter' |
      head -n 1
  } || true)"
...
for certificate_dir in "$old_certificates" "$new_certificates"; do
  ...
  openssl req -x509 -newkey rsa:2048 -nodes -days 1 \
    ...
    -subj "/CN=go-lease-fault-ca" >/dev/null 2>&1
