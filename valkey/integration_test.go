...
func TestLiveBackendConformance(t *testing.T) {
	...
	prefix := "go-lease-integration-" + time.Now().UTC().Format("150405.000000000")
...
func TestLivePartitionOutcome(t *testing.T) {
	...
	t.Cleanup(client.Close)
	store, err := leasevalkey.New(client, "go-lease-partition")
	if err != nil {
		t.Fatalf("New() error = %v", err)
	}
...
func TestLiveFenceContinuity(t *testing.T) {
	...
	store, err := leasevalkey.Open(context.Background(), client, "go-lease-continuity")
	if phase == "reject" {
		assertContinuityRejectionRedacted(t, err)
